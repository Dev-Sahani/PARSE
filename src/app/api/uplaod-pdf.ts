...
export const extractData = async (formData: FormData) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // return { status: 200, ...final_data } as FinalData;
    } else {
  ...
  const url = `/${filename.split(".")[0]}/overview`;
  redirect(url);
};
...
async function uploadToFile(file: string, filename: string) {
  const uploadDir = path.join(process.cwd(), "uploads");
  const filePath = path.join(uploadDir, filename.split(".")[0] + ".txt");
